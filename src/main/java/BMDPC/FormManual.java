 ...
 * @author jimi
 */
public class FormManual extends JMPCFormModal {
    private String xlsType;
    private ManualFormCaller caller=null;
    ...
    class MyCellRenderer extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList list, Object value,
            ...
            String text="";
            if(list.getName().equals("auto")){
                text="<html><div  style='width:100px;margin:5px;padding:5px;border: 2px solid;border-radius: 10px'>"
                        + "<div>"+rows[0].getCells().get(0).getDBValue()+"</div>"
                        + "<div>"+rows[0].getCells().get(12).getDBValue()+"</div>"
                        + "<div>"+rows[0].getCells().get(13).getText()+"</div>"
                        + "</div></html>";
            }else if(list.getName().equals("manual")){
                text="<html><div  style='width:100px;margin:5px;padding:5px;border: 2px solid;border-radius: 10px'>"
                        + "<div>"+rows[0].getCells().get(0).getDBValue()+"</div>"
                        + "<div>"+rows[0].getCells().get(12).getDBValue()+"</div>"
                        + "<div>"+rows[0].getCells().get(13).getText()+"</div>"
     ...
     */
    public FormManual(java.awt.Frame parent, boolean modal, ManualFormCaller caller, String xlsType) {
        super(parent, modal);
        this.caller=caller;
    ...
    private void setInterface(){
        ...
        this.caller.getAutoTable().firstRow(false);
        do{
            JMRow[] rows={this.caller.getAutoTable().getCurrentRow(),null};
            m1.addElement(rows);
        }while(this.caller.getAutoTable().nextRow(false)!=null);
        
        this.caller.getManualTable().firstRow(false);
        do{
            JMRow[] rows={this.caller.getManualTable().getCurrentRow(),null};
            m2.addElement(rows);
        }while(this.caller.getManualTable().nextRow(false)!=null);
    ...
    private void formWindowClosing(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosing
        ...
        if(!this.m2.isEmpty()){
            ...
            if (option != 0)process=false;
        }
        if(process){
            if(!this.m3.isEmpty()){
                List<String> tbAutoKeys=new ArrayList();
                List<String> tbManualKeys=new ArrayList();
                for(Object obj:this.m3.toArray()){
                    JMRow[] rows=(JMRow[]) obj;
                    tbManualKeys.add(rows[1].getCells().get(0).getDBValue());
                    tbAutoKeys.add(rows[0].getCells().get(0).getDBValue());
                }
                this.caller.setResult(tbAutoKeys, tbManualKeys);
            }
        }
        if (process) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                FormManual dialog = new FormManual(new javax.swing.JFrame(), true,null,"");
